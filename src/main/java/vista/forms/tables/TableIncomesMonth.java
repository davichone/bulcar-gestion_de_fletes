 ...
 * @author drola
 */
public class TableIncomesMonth extends javax.swing.JDialog {

    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(TableIncomesMonth.class.getName());
     ...
     */
    public TableIncomesMonth(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        injectTable();
        injectData();
   ...
   private void injectTable() {
    ...
    try {
        List<Income> listaSolicitudes = controllIncome.getlist();
        for (Income i : listaSolicitudes) {
            ...
            fila[4] = i.getDate(); // Pasamos el LocalDate puro (Importante para el Sorter)
            fila[5] = i.getAmount(); 

            modelo.addRow(fila);
        }

        TableIncome.setModel(modelo);
        
        // Configurar Sorter
        TableRowSorter<DefaultTableModel> sorter = new TableRowSorter<>(modelo);
        TableIncome.setRowSorter(sorter);
        
        // Aplicar diseño y alineación
        ajustarColumnas(TableIncome);
    ...
    private void btnExcel1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcel1ActionPerformed
        ExcelExporter.exportar(TableIncome, "Reporte Deudores");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                TableIncomesMonth dialog = new TableIncomesMonth(new javax.swing.JFrame(), true);
